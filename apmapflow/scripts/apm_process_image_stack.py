...
def generate_index_map(nonzero_locs, shape):
    ...
    x_c = np.unravel_index(nonzero_locs, shape)[0].astype(sp.int16)
    y_c = np.unravel_index(nonzero_locs, shape)[1].astype(sp.int16)
    z_c = np.unravel_index(nonzero_locs, shape)[2].astype(sp.int16)
...
def generate_node_connectivity_array(index_map, data_array):
    ...
    for n, sect in enumerate(slice_list):
        ...
        if inds.size:
            ...
            conns = np.append(conns, inds.astype(sp.uint32), axis=0)
...
def generate_adjacency_matrix(conns, nonzero_locs):
    ...
    logger.info(msg)
    mapper = np.ones(nonzero_locs[-1]+1, dtype=sp.uint32) * sp.iinfo(sp.uint32).max
    mapper[nonzero_locs] = np.arange(nonzero_locs.size, dtype=sp.uint32)
...
def remove_isolated_clusters(conns, nonzero_locs, num_to_keep, **kwargs):
    ...
    if kwargs.get('output_img', False):
        save_cluster_image(cs_ids,
                           ...
                           kwargs.get('img_name'))
    #
    inds = np.where(np.in1d(cs_ids, groups[0:num_to_keep]))[0]
...
def save_cluster_image(cs_ids, groups, counts, locs, img_shape, img_name):
    ...
    data = np.ones(cs_ids.size, dtype=sp.uint8) * 255
    ...
    img_data = np.zeros(img_shape, dtype=sp.uint8)
...
def filter_high_gradients(data_map):
    ...
    mag = np.lib.scimath.sqrt(zdir_grad**2 + xdir_grad**2)
    data_map += 1
    data_vector = np.ravel(data_map)
    #
    ...
    #
    data_vector[np.where(cs_ids != cs_num)[0]] = sp.nan
