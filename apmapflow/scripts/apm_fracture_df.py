...
def find_profiles(fracture_slice):
    ...
    profile = np.ones(data.shape, dtype=float) * sp.inf
    profile[a1_coords, a2_coords] = a2_coords
    bottom = np.amin(profile, axis=1)
    bottom[~np.isfinite(bottom)] = sp.nan
    #
    profile = np.ones(data.shape, dtype=float) * -sp.inf
    profile[a1_coords, a2_coords] = a2_coords
    top = np.amax(profile, axis=1)
    top[~np.isfinite(top)] = sp.nan
    #
    mid = (bottom + top)/2.0
    #
...
def df_best_fit(df_data):
    ...
    #
    # setting up vectors
    log_band = np.lib.scimath.log(df_data[:, 0])
    log_std_dev = np.lib.scimath.log(df_data[:, 1])
    #
