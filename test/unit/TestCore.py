...
class TestCore:
    ...
    def test_fracture_image_stack(self):
        ...
        fracture_stack = apm.FractureImageStack(img_data, dtype=np.uint8)
        assert issubclass(fracture_stack.__class__, np.ndarray)
        assert fracture_stack.dtype == np.uint8
